...
async function getBriefingArchive(params: ArchivePageProps['searchParams']): Promise<BriefingArchive> {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/briefings/archive?${searchParams}`, {
...
async function getArchiveStats(): Promise<{
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/briefings/stats`, {
...
export const metadata: Metadata = {
  ...
  description: 'Browse the complete archive of Terry\'s unhinged briefings. Months of magnificent chaos, organized for your browsing pleasure.',
  openGraph: {
    title: 'Briefing Archive - Transfer Juice',
    description: 'Browse the complete archive of Terry\'s unhinged briefings. Months of magnificent chaos, organized for your browsing pleasure.',
    type: 'website',
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center space-y-6">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
              ...
              Every unhinged briefing, every ascerbic observation, every moment of beautiful madness.
            </p>
            ...
            <div className="stats-grid grid grid-cols-2 md:grid-cols-4 gap-6 max-w-3xl mx-auto mt-8">
              <div className="stat-card bg-card border border-border rounded-lg p-4 text-center">
                <div className="text-2xl font-black text-orange-500">{stats.totalBriefings}</div>
                <div className="text-sm text-muted-foreground">Total Briefings</div>
              </div>
              <div className="stat-card bg-card border border-border rounded-lg p-4 text-center">
                <div className="text-2xl font-black text-orange-500">{stats.averageTerryScore}%</div>
                <div className="text-sm text-muted-foreground">Avg Terry Score</div>
              </div>
              <div className="stat-card bg-card border border-border rounded-lg p-4 text-center">
                <div className="text-2xl font-black text-orange-500">{stats.topClubs.length}</div>
                <div className="text-sm text-muted-foreground">Clubs Covered</div>
              </div>
              <div className="stat-card bg-card border border-border rounded-lg p-4 text-center">
                <div className="text-2xl font-black text-orange-500">{stats.topPlayers.length}</div>
                <div className="text-sm text-muted-foreground">Players Mentioned</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <aside className="archive-sidebar lg:col-span-1">
            <div className="sticky top-8 space-y-6">
              ...
              <div className="filter-card bg-card border border-border rounded-lg p-6">
                <h3 className="text-lg font-bold text-foreground mb-4">Find Briefings</h3>
                ...
                <form className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {['PL', 'LaLiga', 'SerieA', 'Bundesliga', 'Ligue1'].map(league => (
                        <label key={league} className="flex items-center">
                          <input 
                            ...
                            value={league}
                            className="mr-2"
                            defaultChecked={searchParams.leagues?.includes(league)}
                          />
              ...
              <div className="top-content-card bg-card border border-border rounded-lg p-6">
                <h3 className="text-lg font-bold text-foreground mb-4">Most Mentioned</h3>
                
                <div className="space-y-4">
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground mb-2">Clubs</h4>
              ...
              <div className="newsletter-card">
                <NewsletterSignup 
                  variant="sidebar"
                  headline="Never Miss a Briefing"
                  description="Get Terry's daily chaos delivered"
                  compact={true}
                />
              </div>
            </div>
          ...
          <main className="briefings-grid lg:col-span-3">
            {archive.briefings.length === 0 ? (
              <div className="empty-state text-center py-12">
                ...
                <h2 className="text-2xl font-bold text-foreground mb-2">No Briefings Found</h2>
                <p className="text-muted-foreground mb-6">
                  Try adjusting your filters or check back later for new briefings.
                </p>
                <Link 
                  href="/briefings"
              ...
              <>
                ...
                <div className="results-header mb-8">
                  ...
                  <p className="text-muted-foreground">
                    Showing page {currentPage} of {Math.ceil(archive.pagination.total / archive.pagination.limit)}
                  </p>
                </div>
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="briefing-meta">
                          <div className="text-sm text-muted-foreground font-mono mb-1">
                            {briefing.title.day} {briefing.title.hour} • {briefing.title.month} {briefing.title.year}
                          </div>
                  ...
                  <nav className="pagination mt-12">
                    <div className="flex items-center justify-center space-x-4">
                      ...
                      <span className="text-muted-foreground">
                        Page {currentPage} of {Math.ceil(archive.pagination.total / archive.pagination.limit)}
                      </span>
