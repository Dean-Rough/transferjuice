import { EnhancedStory } from './enhancedBriefingGenerator';
...
export function analyzeBriefingContent(stories: any[]): BriefingAnalysis {
  ...
  stories.forEach((storyData, index) => {
    ...
    const playerMatch = story.headline.match(/([A-Z][a-z]+ [A-Z][a-zöäüßéè]+)/g);
    if (playerMatch) {
      analysis.keyPlayers.push(...playerMatch);
    }

    // Extract club names
    const clubPattern = /(Arsenal|Chelsea|Liverpool|Manchester United|Manchester City|Tottenham|Newcastle|West Ham|Bayern Munich|Real Madrid|Barcelona|PSG|Juventus|Milan|Inter|Dortmund)/gi;
    const clubMatches = (story.headline + ' ' + story.contextParagraph).match(clubPattern);
    if (clubMatches) {
    ...
    if (index === 0) {
      ...
      const fromMatch = story.contextParagraph.match(/from ([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/);
      const toMatch = story.contextParagraph.match(/to ([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/);
...
export function generateOptimizedHeadline(analysis: BriefingAnalysis, date: Date): string {
...
export function generateMetaDescription(analysis: BriefingAnalysis): string {
  ...
  if (analysis.secondaryStories.length > 0) {
    parts.push(`Plus updates on ${analysis.secondaryStories.slice(0, 2).join(' and ')}`);
  }
...
export function generateSEOMetadata(
  ...
  return {
    ...
    keywords: [
      ...
      'transfer news',
      'football transfers',
      publishDate.toLocaleDateString('en-GB', { month: 'long', year: 'numeric' }) + ' transfers'
    ],
