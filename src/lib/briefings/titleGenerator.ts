...
export class TitleGenerator {
  private static readonly TITLE_TEMPLATES: TitleTemplate[] = [
    ...
    {
      pattern:
        "{player} to {club} for €{fee}m (Because Apparently Money Has No Meaning)",
    ...
    {
      pattern:
        "{player} Does Something {adjective} (Shocking Absolutely No One)",
  ...
  private static readonly VARIABLE_GENERATORS = {
    ...
    number: ["47", "73", "23", "89", "156", "12", "99", "42", "17", "666"],
  };
  ...
  static generateTitle(timestamp: Date, context: TitleContext): BriefingTitle {
    ...
    const viableTemplates = scoredTemplates.filter(
    ...
    const funnyTitle = this.fillTemplate(selectedTemplate, context);
  ...
  static generateTitleOptions(
    ...
    for (
      let i = 0;
      i < count && usedPatterns.size < availableTemplates.length;
      i++
    ) {
      ...
      usedPatterns.add(selectedTemplate.pattern);
   ...
   */
  static extractContextFromSections(
    sections: BriefingSection[],
  ): Partial<TitleContext> {
    ...
    sections.forEach((section) => {
      ...
      const clubMatches = section.content.match(
        /\b(Arsenal|Chelsea|Manchester United|Liverpool|Real Madrid|Barcelona|Bayern Munich|PSG|Juventus|AC Milan|Inter Milan|Tottenham|Manchester City|Newcastle|Brighton|Aston Villa|West Ham)\b/gi,
      );
      if (clubMatches) {
      ...
      if (feeMatches) {
        const fees = feeMatches.map((match) =>
          parseFloat(match.replace(/[£€m]/g, "")),
        );
      ...
      if (
        section.content.toLowerCase().includes("breaking") ||
  ...
  private static filterTemplatesByTime(timeOfDay: string): TitleTemplate[] {
    return this.TITLE_TEMPLATES.filter(
      (template) =>
  ...
  private static selectWeightedTemplate(
    scoredTemplates: Array<{ template: TitleTemplate; score: number }>,
  ): TitleTemplate {
    const totalWeight = scoredTemplates.reduce(
      (sum, scored) => sum + scored.score,
      0,
    );
    ...
    switch (variable) {
      ...
      case "fee":
        return context.fees && context.fees.length > 0
          ? context.fees[
              Math.floor(Math.random() * context.fees.length)
            ].toString()
      ...
      default:
        ...
        const options =
          this.VARIABLE_GENERATORS[
            variable as keyof typeof this.VARIABLE_GENERATORS
          ];
        if (options) {
  ...
  private static formatBriefingTitle(
    timestamp: Date,
    funnyTitle: string,
  ): BriefingTitle {
    const days = [
