 ...
 */
export async function generateDailyBreakingNews(): Promise<BreakingNewsStory[]> {
  const stories: BreakingNewsStory[] = [];
  const usedTemplates = new Set<number>();
  
  // Generate 6 unique stories
  while (stories.length < 6) {
    const templateIndex = Math.floor(Math.random() * BREAKING_NEWS_TEMPLATES.length);
    
    // Ensure we don't use the same template twice
    if (usedTemplates.has(templateIndex) && usedTemplates.size < BREAKING_NEWS_TEMPLATES.length) {
      continue;
    }
    ...
    // Add Terry's quip for some stories (not all, for variety)
    if (Math.random() > 0.5) {
      try {
        const terryResponse = await generateTerryCommentary(
          `React to this ridiculous transfer news: "${story.headline}"`,
          {
            style: 'sarcastic',
            length: 'short',
        ...
        story.terryQuip = terryResponse.text;
      } catch (error) {
        // If Terry fails, use a fallback quip
        story.terryQuip = getRandomTerryQuip();
      }
    }
 ...
 */
export function formatBreakingNewsForDisplay(stories: BreakingNewsStory[]): string {
