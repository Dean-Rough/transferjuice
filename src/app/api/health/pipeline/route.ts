...
async function performHealthChecks(services?: string[]): Promise<any[]> {
  const servicesToCheck = services || ['database', 'ai', 'twitter', 'websocket', 'cache'];
...
function analyzeExecutionHealth(executions: any[]): {
  ...
  if (successRate >= 90 && timeSinceLastExecution < 7200000) { // 2 hours
    status = 'healthy';
...
async function runPipelineTest(): Promise<any[]> {
  const tests = [
    ...
    },
    {
      ...
      test: async () => {
        const sources = await prisma.itkSource.count({ where: { isActive: true } });
        return sources > 0;
      },
    },
    {
      ...
      test: async () => {
        return CONFIG.quality.thresholds.terryVoice >= 0.75 && 
               CONFIG.quality.autoPublishThreshold >= 0.80;
      },
    },
  ];
