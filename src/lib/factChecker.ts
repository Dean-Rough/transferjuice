...
const SUSPICIOUS_PATTERNS = [
  // Unrealistic fees
  { pattern: /£(\d+)m/, check: (match: string) => {
    const amount = parseInt(match.replace(/[£m]/g, ''));
    return amount > 300 ? `Suspicious transfer fee: ${match} seems unrealistic` : null;
  }},
  
  // Future dates
  { pattern: /(?:will join|to join|joining) (?:in|after) (\w+ \d{4})/, check: (match: string) => {
    if (match.includes('August') || match.includes('July')) return null;
    return `Future transfer date seems too far: ${match}`;
  }},
  
  // Retired players
  { pattern: /Lionel Messi|Cristiano Ronaldo/, check: (match: string) => {
    return `Be cautious with aging superstar rumors`;
  }}
];

export function factCheckStory(content: string, metadata?: any): FactCheckResult {
  const issues: string[] = [];
  ...
  for (const [manager, correctClub] of Object.entries(KNOWN_MANAGERS)) {
    const managerPattern = new RegExp(`${manager}.*(?:manager|coach|confirms)`, 'i');
    if (managerPattern.test(content)) {
      // Check if wrong club is mentioned
      const clubPattern = /(?:AC Milan|Inter Milan|Chelsea|Arsenal|Barcelona|PSG|Bayern Munich)/i;
      const mentionedClub = content.match(clubPattern)?.[0];
      
      if (mentionedClub && mentionedClub !== correctClub && correctClub !== 'Retired') {
        issues.push(`Manager mismatch: ${manager} is at ${correctClub}, not ${mentionedClub}`);
  ...
  if (content.includes('confirmed') && content.includes('might') && content.includes('could')) {
    issues.push('Mixed signals: Story contains both confirmed and speculative language');
  }
  ...
  if (metadata?.payCut && typeof metadata.payCut === 'string') {
    ...
    if (payCutMatch) {
      ...
      if (percentage > 50) {
        issues.push(`Unrealistic pay cut: ${percentage}% reduction seems excessive`);
      }
    }
  }
...
export function validateRSSItem(item: any): boolean {
  ...
  if (item.content_text.startsWith('RT by') || item.content_text.trim() === '') {
    return false;
  }
  ...
  if (item.content_text.toLowerCase().includes('bet now') || 
...
export function extractFactualContent(content: string): string {
  ...
  const emojiPattern = /^[\s\u{1F300}-\u{1F9FF}\u{1F600}-\u{1F64F}\u{1F680}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]+/u;
