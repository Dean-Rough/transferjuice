...
export function PremiumSharingModule({
  briefing,
  className = "",
}: PremiumSharingModuleProps) {
  const [shareCount, setShareCount] = useState(briefing.sharing.shareCount);
  const [selectedQuote, setSelectedQuote] = useState(
    briefing.sharing.quotes[0] || "",
  );
  ...
  const shareToTwitter = async () => {
    setIsSharing(true);
    window.open(
      sharePlatforms.find((p) => p.id === "twitter")?.url,
      "_blank",
  ...
  const shareToLinkedIn = async () => {
    setIsSharing(true);
    window.open(
      sharePlatforms.find((p) => p.id === "linkedin")?.url,
      "_blank",
      "width=550,height=420",
    );
  ...
  const shareViaEmail = async () => {
    ...
    window.location.href =
      sharePlatforms.find((p) => p.id === "email")?.url || "";
  ...
  const copyToClipboard = async () => {
    ...
    try {
      ...
      setTimeout(
        () => setRecentShares((prev) => prev.filter((s) => s !== "copy")),
        2000,
      );
  ...
  return (
    <div
      className={`premium-sharing-module mt-12 pt-8 border-t border-border ${className}`}
      data-testid="premium-sharing-module"
    >
      {/* Section Header */}
      <div className="sharing-header text-center mb-8">
        ...
        <p className="text-muted-foreground max-w-md mx-auto">
          Spread the beautiful madness of transfer journalism. Your friends will
          thank you (or block you).
        </p>
        {getTotalShares() > 0 && (
        ...
        <div className="quote-card terry-bubble bg-orange-600 text-white p-6 max-w-2xl mx-auto">
          ...
          <div className="quote-controls flex items-center justify-between">
            ...
            <button
              onClick={() =>
                handleQuoteSelect(
                  briefing.sharing.quotes[
                    Math.floor(Math.random() * briefing.sharing.quotes.length)
                  ],
                )
              }
      ...
      <div className="newsletter-integration mb-8">
        <div className="newsletter-cta bg-gradient-to-r from-orange-600/10 to-orange-600/5 border border-orange-600/20 rounded-lg p-6 max-w-2xl mx-auto">
          ...
          <NewsletterSignup
            variant="inline"
            className={`briefing-${briefing.id}`}
          />
        </div>
      </div>
        ...
        <div className="actions-grid grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">
              Dive into the archive of magnificent chaos
            </p>
          </a>
          ...
          >
            ...
            <h4 className="font-bold text-foreground mb-1">
              Follow Transfer Juice
            </h4>
            <p className="text-sm text-muted-foreground">
              Real-time Terry wisdom on Twitter
            </p>
          </a>
          ...
          <button
            onClick={() =>
              window.open(
                "mailto:feedback@transferjuice.com?subject=Briefing Feedback",
                "_blank",
              )
            }
            className="action-card card p-4 text-center hover:border-orange-600/30 transition-all duration-200 group"
          >
            ...
            <p className="text-sm text-muted-foreground">
              Tell Terry what you really think
            </p>
          </button>
        </div>
      </div>
