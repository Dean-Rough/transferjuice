import { AggregatedStory, ExtractedFacts, compareFacts } from './storyAggregator';
...
function generateConfirmedNarrative(story: AggregatedStory): string {
  ...
  if (includeContext) {
    const clubContext = CLUB_CONTEXT[buyingClub];
    if (clubContext && leadFact.position && clubContext.needs.includes(leadFact.position)) {
  ...
  if ((agreed.contractLength || leadFact.contractUntil) && narrativeIndex % 2 === 0) {
    narrative += ` ${agreed.contractLength || `Contract until ${leadFact.contractUntil}`}. `;
  }
  
  // Vary the ending style based on story details
  const feeValue = agreed.fee ? parseFloat(agreed.fee.match(/[\d.]+/)?.[0] || '0') : 0;
...
function generateInterestNarrative(story: AggregatedStory): string {
  ...
  if (interestedClubs.length === 1) {
    ...
    narrative += singleClubEndings[Math.floor(Math.random() * singleClubEndings.length)];
  } else {
    ...
    narrative += multiClubEndings[Math.floor(Math.random() * multiClubEndings.length)];
  }
...
export function generateRoundupParagraph(stories: AggregatedStory[]): string {
  ...
  let narrative = "Meanwhile, in today's collection of spurious links and agent-manufactured interest: ";
  ...
  narrative += ". The transfer window: where journalism goes to die and agents go to get rich.";
