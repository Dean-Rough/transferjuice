...
export async function getPlayerContext(playerName: string): Promise<PlayerContext> {
  ...
  try {
    ...
    if (recentMentions) {
      const statsMatch = recentMentions.match(/(\d+)\s+goals?\s+(?:this\s+season|in\s+\d+\s+games?)/i);
      if (statsMatch) {
    ...
    const contextKeywords = {
      ...
      situation: ['contract expires', 'final year', 'release clause', 'unsettled']
    };
...
async function searchRecentMentions(playerName: string): Promise<string | null> {
  try {
    // In practice, we'd search through recent RSS items we've collected
    // Looking for mentions of the player in the last week
    const response = await fetch('https://rss.app/feeds/v1.1/_zMqruZvtL6XIMNVY.json');
...
export function getClubContext(clubName: string, text: string): string | null {
  ...
  const positionMatch = text.match(new RegExp(`${clubName}.*?(\\d+)(?:st|nd|rd|th)\\s+(?:place|position)`, 'i'));
  const managerMatch = text.match(new RegExp(`${clubName}.*?(?:manager|boss|coach)\\s+([A-Z][a-z]+\\s+[A-Z][a-z]+)`, 'i'));
...
export async function searchRecentContext(playerName: string): Promise<string | null> {
