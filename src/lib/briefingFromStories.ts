...
import {
  getStoriesForBriefing,
  getMegaStoriesForRecap,
} from "./storyProcessor";
import {
...
export async function generateBriefingFromStories(
  ...
  const keyPlayers = [...new Set(aggregatedStories.map((s) => s.player))].slice(
    0,
    8,
  );
  const keyClubs = [
    ...new Set(aggregatedStories.flatMap((s) => s.primaryClubs)),
  ].slice(0, 5);
...
<figcaption>${mainImagePlayer}</figcaption>
  ...
  const tier2Stories = aggregatedStories.filter(
  ...
  if (tier1Stories.length > 0) {
    for (let i = 0; i < Math.min(tier1Stories.length, 3); i++) {
      ...
      if (i === 0) {
        ...
        const meta = stories.find(
          (s) => (s.metadata as any).player === story.player,
        )?.metadata as any;
...
function generateRelevantSectionHeader(stories: AggregatedStory[]): string {
  const confirmedCount = stories.filter(
    (s) => s.storyType === "confirmed",
  ).length;
  const negotiatingCount = stories.filter(
    (s) => s.storyType === "negotiating",
  ).length;

  if (confirmedCount === stories.length) {
    return confirmedCount === 1
      ? `${stories[0].player} Deal Complete`
      : "More Confirmed Moves";
  }
...
function generateBriefingTitle(
