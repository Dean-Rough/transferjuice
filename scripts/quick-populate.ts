  ...
  try {
      ...
      if (!existing) {
        const source = await prisma.iTKSource.create({
          data: {
            ...
            region: account.specialties.includes('Global') ? 'GLOBAL' : 
                    ...
                    account.specialties.includes('Premier League') ? 'UK' : 
                    account.specialties.includes('La Liga') ? 'ES' :
                    account.specialties.includes('Germany') ? 'DE' :
                    account.specialties.includes('France') ? 'FR' :
                    account.specialties.includes('Brazil') ? 'BR' : 'GLOBAL',
            isActive: true,
            fetchInterval: 900, // 15 minutes
            followerCount: Math.floor(Math.random() * 5000000) + 100000,
          },
        });
        console.log(`✅ Added source: ${account.displayName} (@${account.username})`);
        ...
        for (let i = 0; i < itemCount; i++) {
          ...
          await prisma.feedItem.create({
            data: {
              type: 'ITK',
              content: transferNews[Math.floor(Math.random() * transferNews.length)],
              sourceId: source.id,
              ...
              isProcessed: true,
              isPublished: true,
              transferType: ['RUMOUR', 'BID', 'MEDICAL', 'PERSONAL_TERMS'][Math.floor(Math.random() * 4)] as any,
              league: source.region === 'UK' ? 'PL' : 
                      source.region === 'ES' ? 'LALIGA' :
                      source.region === 'IT' ? 'SERIEA' :
                      source.region === 'DE' ? 'BUNDESLIGA' :
                      source.region === 'FR' ? 'LIGUE1' : null,
    ...
    for (const item of uncommentedItems) {
      await prisma.feedItem.update({
        where: { id: item.id },
        data: {
          terryCommentary: terryQuips[Math.floor(Math.random() * terryQuips.length)],
        },
      });
    }

    console.log(`✅ Added Terry commentary to ${uncommentedItems.length} items`);

    // Step 3: Create some tags
    console.log('\n🏷️  Creating tags...');
    
    const clubs = ['Arsenal', 'Chelsea', 'Liverpool', 'Man United', 'Man City', 'Real Madrid', 'Barcelona', 'Bayern Munich', 'PSG', 'Juventus'];
    const players = ['Mbappe', 'Haaland', 'Bellingham', 'Rice', 'Osimhen', 'Kane', 'Saka', 'Vinicius', 'Pedri', 'Musiala'];
    ...
    const allPlayerTags = await prisma.tag.findMany({ where: { type: 'PLAYER' } });

    for (const item of allItems) {
      // Add 1-2 club tags
      const clubCount = Math.floor(Math.random() * 2) + 1;
      for (let i = 0; i < clubCount; i++) {
        const randomClub = allClubTags[Math.floor(Math.random() * allClubTags.length)];
      ...
      // Add 0-1 player tags
      if (Math.random() > 0.5) {
        const randomPlayer = allPlayerTags[Math.floor(Math.random() * allPlayerTags.length)];
        await prisma.feedItemTag.upsert({
          where: {
            feedItemId_tagId: {
              feedItemId: item.id,
