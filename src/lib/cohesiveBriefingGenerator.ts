...
import {
  generateGolbyNarrative,
  generateSectionHeader,
  generateRoundupParagraph,
} from "./golbyNarrativeGenerator";
    ...
    if (response.ok) {
      ...
      if (preferLandscape && data.originalimage) {
        ...
        if (data.originalimage.width && data.originalimage.height) {
          const aspectRatio =
            data.originalimage.width / data.originalimage.height;
          // Only use if it's reasonably landscape (aspect ratio > 0.8)
          if (aspectRatio > 0.8) {
            imageUrl = data.originalimage.source;
          } else {
            console.log(
              `Skipping portrait image for ${playerName} (aspect ratio: ${aspectRatio})`,
            );
            return null;
          }
        } else {
...
async function getClubBadge(clubName: string): Promise<string | null> {
  ...
  try {
    const wikiName =
      clubMappings[clubName] || clubName.replace(" ", "_") + "_F.C.";
    const response = await fetch(
      `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(wikiName)}`,
    );
...
export async function generateCohesiveBriefing(
  ...
  const factCheckedStories = aggregatedStories.filter((story) => {
    ...
    if (!factCheck.isValid) {
      console.log(
        `⚠️ Fact check failed for ${story.player}: ${factCheck.issues.join(", ")}`,
      );
      // Skip stories with major fact-checking issues
      if (
        factCheck.issues.some(
          (issue) =>
  ...
  if (factCheckedStories.length === 0) {
    ...
    return {
      ...
      content:
        "<p>We're experiencing some technical difficulties with our news sources. Please check back later for the latest transfer updates.</p>",
      metadata: {
        keyPlayers: [],
        keyClubs: [],
        mainImage: undefined,
  ...
  const keyClubs = [
    ...new Set(factCheckedStories.flatMap((s) => s.primaryClubs)),
  ].slice(0, 5);
  ...
  const tier2Stories = factCheckedStories.filter(
    (s) => s.importance >= 4 && s.player !== mainImagePlayer,
  );
  ...
  return generateAggregatedBriefing(
    factCheckedStories,
    keyPlayers,
    keyClubs,
    mainImage,
  ...
  const formatContent = (text: string): string => {
    ...
    keyPlayers.forEach((player) => {
      if (player) {
        ...
        text = text.replace(
          regex,
          `<strong class="text-orange-500">${player}</strong>`,
        );
      }
    });
    ...
    keyClubs.forEach((club) => {
      ...
      text = text.replace(
        regex,
        `<strong class="text-orange-500">${club}</strong>`,
      );
    });
...
<figcaption>${keyClubs[0]}</figcaption>
  ...
  const tier2Stories = stories.filter(
    (s) => s.importance >= 4 && s.importance < 7,
  ); // Medium
    ...
    for (let i = 0; i < tier1Stories.length; i++) {
      ...
      if (i === 0) {
        ...
        if (story.storyType === "confirmed" && story.primaryClubs.length > 0) {
          ...
          story.storyType === "negotiating" &&
          story.primaryClubs.length > 0
        ) {
          ...
          storyHeader = hasPacut
            ? `${story.player} Takes Pay Cut to Stay`
            : `${story.player} Signs New Deal`;
        } else if (
          story.storyType === "rejected" &&
          story.primaryClubs.length > 0
        ) {
          storyHeader = `${story.player} Rejects ${story.primaryClubs[0]}`;
        } else if (
          story.storyType === "interest" &&
          story.primaryClubs.length > 0
        ) {
  ...
  if (tier2Stories.length > 0) {
    ...
    const tier2Image = playerImages
      ? Object.values(playerImages)[0]
      : undefined;
  ...
  if (tier3Stories.length > 0) {
    ...
    const interestStories = tier3Stories.filter(
      (s) => s.storyType === "interest",
    );
    const rejectedStories = tier3Stories.filter(
      (s) => s.storyType === "rejected",
    );
...
function generateRelevantSectionHeader(stories: AggregatedStory[]): string {
  ...
  const confirmedStories = stories.filter((s) => s.storyType === "confirmed");
  const negotiatingStories = stories.filter(
    (s) => s.storyType === "negotiating",
  );
  ...
  if (confirmedStories.length === stories.length) {
    if (confirmedStories.length === 1) {
      ...
      const clubs = [
        ...new Set(confirmedStories.flatMap((s) => s.primaryClubs[0])),
      ];
      if (clubs.length === 1) {
  ...
  if (negotiatingStories.length === stories.length) {
    if (negotiatingStories.length === 1) {
      ...
      const clubs = [
        ...new Set(negotiatingStories.map((s) => s.primaryClubs[0])),
      ];
      if (clubs.length === 1) {
  ...
  if (
    leadStory.storyType === "confirmed" &&
    leadStory.primaryClubs.length > 0
  ) {
    return `${leadStory.player} to ${leadStory.primaryClubs[0]} Plus More`;
  } else if (
    leadStory.storyType === "negotiating" &&
    leadStory.primaryClubs.length > 0
  ) {
