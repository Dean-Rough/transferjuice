...
import type { ITKSource } from "@/types/sources";
import {
  ...
  for (const source of GLOBAL_ITK_SOURCES) {
    try {
      await upsertITKSource({
        ...
        isVerified: (source as any).isVerified || false,
      });
    } catch (error) {
...
export async function fetchTweetsForBriefing(
  ...
  console.log(
    `✅ Fetched ${results.totalTweets} tweets, ${results.relevantTweets} relevant`,
  );
...
export async function getTweetsForBriefing(timestamp: Date): Promise<{
  ...
  // Fetch tweets from the last 24 hours for now (since we have limited data)
  // TODO: Change back to 1 hour when we have continuous monitoring
