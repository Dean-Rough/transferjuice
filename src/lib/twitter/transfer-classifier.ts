...
class TransferClassifier {
  ...
  async classifyTweet(
    ...
    if (confidence >= 0.7) {
      ...
      explanation = applyTerryStyle.enhanceMessage(
        `Strong transfer indicators detected: ${keywords.slice(0, 3).join(', ')}`
      );
    } else if (confidence >= 0.4) {
      reasonCode = 'medium_confidence';
      explanation = applyTerryStyle.enhanceMessage(
        `Moderate transfer signals: ${keywords.slice(0, 2).join(', ')}`
      );
    } else {
      reasonCode = 'low_confidence';
      explanation = applyTerryStyle.enhanceMessage(
        'Minimal transfer relevance detected'
      );
    }
  ...
  private determineTransferType(
    ...
    switch (keywordType) {
      case 'confirmed':
        return confidence > 0.8 ? 'OFFICIAL' : 'CONFIRMED';
      case 'medical':
        return 'MEDICAL';
      case 'advanced':
        return 'ADVANCED';
