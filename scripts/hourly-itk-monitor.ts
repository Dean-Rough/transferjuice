...
async function hourlyMonitoring() {
  ...
  console.log(`Sources: ${ITK_SOURCES_CONFIG.sources.length} accounts across ${ITK_SOURCES_CONFIG.regions.length} regions\n`);
    ...
    for (const source of region.sources) {
      ...
      try {
        ...
        for (const tweet of tweets) {
          ...
          const transferKeywords = ['transfer', 'signing', 'medical', 'fee', 'deal', 'agreed', 'bid', 'contract', 'loan', 'here we go'];
  ...
  console.log(`Sources monitored: ${results.successfulSources}/${results.totalSources}`);
  console.log(`Total tweets checked: ${results.totalTweets}`);
  console.log(`Transfer-related: ${results.transferTweets} (${((results.transferTweets / results.totalTweets) * 100).toFixed(1)}%)`);
  ...
  // Save monitoring log
  const logPath = path.join(process.cwd(), 'logs', `monitoring-${new Date().toISOString().split('T')[0]}.json`);
