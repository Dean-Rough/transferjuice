...
function classifyTweet(tweet) {
  ...
  const keywords = ['breaking', 'fee', 'agree', 'medical', 'personal terms', 'confirmed', 'official', 'here we go'];
  ...
  const baseConfidence = tweet.source.tier === 1 ? 0.8 : tweet.source.tier === 2 ? 0.6 : 0.4;
...
function extractEntities(text) {
  ...
  const clubs = ['Arsenal', 'Real Madrid', 'Juventus', 'Bayern Munich', 'Manchester United', 'Barcelona'];
  const players = ['Declan Rice', 'Kylian Mbappé', 'Victor Osimhen', 'Jamal Musiala', 'Pedri'];
...
function generateTerryCommentary(tweet, classification) {
  ...
  let templates;
  if (classification.transferType === 'confirmed' && tweet.text.includes('BREAKING')) {
...
function validateVoiceConsistency(commentary) {
  ...
  const ascerbicIndicators = ['either', 'or', 'question is', 'probably', 'apparently', 'basically'];
  const britishIndicators = ['right,', 'brilliant', 'mental', 'proper'];
  const footballIndicators = ['medical', 'personal terms', 'fee', 'training ground'];
  const golbyIndicators = ['coffee machine', 'car keys', 'horoscope', 'heat death'];
  
  const ascerbicScore = ascerbicIndicators.filter(i => text.includes(i)).length / 3;
  const britishScore = britishIndicators.filter(i => text.includes(i)).length / 2;
  const footballScore = footballIndicators.filter(i => text.includes(i)).length / 2;
  const golbyScore = golbyIndicators.filter(i => text.includes(i)).length / 1;
  
  const overallConsistency = (ascerbicScore * 0.3 + britishScore * 0.25 + footballScore * 0.25 + golbyScore * 0.2);
...
function processTweet(tweet) {
  ...
  if (shouldComment && terryCommentaryCount < 8) { // Max 8 per "hour"
    const commentary = generateTerryCommentary(tweet, classification);
    const voiceMetrics = validateVoiceConsistency(commentary);
    
    if (voiceMetrics.overallConsistency >= 0.6) { // Voice quality threshold
      ...
      console.log(`   🎭 Terry commentary rejected (low voice quality: ${Math.round(voiceMetrics.overallConsistency * 100)}%)`);
    }
  } else {
...
function displayResults() {
  ...
  if (itemsWithTerry.length > 0) {
    const avgVoiceConsistency = itemsWithTerry.reduce((sum, item) => sum + item.voiceMetrics.overallConsistency, 0) / itemsWithTerry.length;
    console.log(`- Average Terry voice consistency: ${Math.round(avgVoiceConsistency * 100)}%`);
  }
