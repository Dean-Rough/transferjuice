...
export class PolaroidFrameGenerator {
  private static readonly CACHE_DIR = path.join(process.cwd(), 'public', 'generated-frames');
  private static readonly FONT_PATH = path.join(process.cwd(), 'public', 'fonts', 'Diary-Notes.otf');
  ...
  private static async createPolaroidFrame(
    ...
    try {
      ...
      const { frameWidth, frameHeight, imageWidth, imageHeight } = this.calculateDimensions(
        originalWidth,
        originalHeight,
        options
      );
      ...
      const imageY = (frameHeight - imageHeight - this.getBottomSpaceHeight(frameHeight, options)) / 2;
      ...
      await this.addHandwrittenText(ctx, playerName, frameWidth, frameHeight, options);
   ...
   */
  private static getBottomSpaceHeight(frameHeight: number, options: FrameOptions): number {
   ...
   */
  private static generateCacheKey(playerName: string, options: FrameOptions): string {
   ...
   */
  private static async getCachedFrame(cacheKey: string, playerName: string): Promise<GeneratedFrame | null> {
    try {
   ...
   */
  static async cleanupCache(maxAge: number = 7 * 24 * 60 * 60 * 1000): Promise<number> {
    try {
...
export const generatePlayerFrame = (playerName: string, options?: Partial<FrameOptions>) =>
...
export const getFrameCacheStats = () =>
  PolaroidFrameGenerator.getCacheStats();