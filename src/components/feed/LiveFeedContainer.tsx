...
export function LiveFeedContainer() {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        const transformedItems: FeedUpdate[] = data.data.map((item: any) => ({
          ...
          priority: (item.priority?.toLowerCase() || 'medium') as 'breaking' | 'high' | 'medium' | 'low'
        }));
  ...
  const addBreakingNews = (breakingNews: FeedUpdate) => {
    ...
    setFeedItems(prevItems => {
      const updatedItems = [{ ...breakingNews, type: 'breaking_news' as const }, ...prevItems];
      updateAvailableTags(updatedItems);
      return updatedItems;
    });
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <div className="text-muted-foreground">Loading live transfer feed...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="live-feed-container">
      ...
      <div className={`px-4 py-2 text-sm text-center transition-all ${
        ...
        {connectionStatus === 'connected' && (
          <>
            🟢 Live feed active {lastUpdateTime && `• Last update: ${lastUpdateTime.toLocaleTimeString()}`}
          </>
        )}
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black">
              {activeTags.length > 0 ? '🎯 Filtered Feed' : '🌍 Live Global Transfer Feed'}
            </h2>
      ...
      <div ref={feedContainerRef} className="divide-y divide-border">
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item, index) => {
            ...
            const feedItemData = {
              id: item.id,
              type: item.type === 'breaking_news' ? 'breaking' as const : 
                   item.type === 'story_mix' ? 'partner' as const :
                   'itk' as const,
      ...
      {filteredItems.length > 0 && (
        <div className="p-8 text-center">
          <div className="text-muted-foreground text-sm">
            🔄 Monitoring {availableTags.reduce((acc, tag) => acc + tag.count, 0)} sources globally
          </div>
