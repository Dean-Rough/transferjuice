...
function generateTerryCommentary(tweet, classification) {
  ...
  if (
    ...
    tweet.text.toLowerCase().includes("breaking")
  ) {
...
function validateVoiceConsistency(commentary) {
  ...
  const ascerbicScore = Math.min(
    ascerbicIndicators.filter((i) => text.includes(i)).length / 2,
    1.0,
  ); // More forgiving
  const britishScore = Math.min(
    britishIndicators.filter((i) => text.includes(i)).length / 1.5,
    1.0,
  ); // More forgiving
  const footballScore = Math.min(
    footballIndicators.filter((i) => text.includes(i)).length / 1.5,
    1.0,
  ); // More forgiving
  const golbyScore = Math.min(
    golbyIndicators.filter((i) => text.includes(i)).length / 1,
    1.0,
  );
...
function processTweet(tweet) {
  ...
  if (!classification.isTransferRelated || classification.confidence < 0.3) {
    // Lower threshold
  ...
  if (shouldComment && terryCommentaryCount < 10) {
    // Increased quota for demo
    ...
    if (voiceMetrics.overallConsistency >= 0.4) {
      ...
      voiceAnalytics.avgConsistency =
        (voiceAnalytics.avgConsistency * (voiceAnalytics.totalAccepted - 1) +
          voiceMetrics.overallConsistency) /
        voiceAnalytics.totalAccepted;
      ...
      console.log(
        `   🎭 Terry commentary rejected (voice quality: ${Math.round(voiceMetrics.overallConsistency * 100)}%)`,
      );
    }
  } else {
    console.log(
...
function displayResults() {
  ...
  console.log(
    `- Success rate: ${Math.round((voiceAnalytics.totalAccepted / voiceAnalytics.totalGenerated) * 100)}%`,
  );
  console.log(
    `- Average voice consistency: ${Math.round(voiceAnalytics.avgConsistency * 100)}%`,
  );
  ...
  const humorStats = feedItems
    .filter((item) => item.humorLevel)
    .reduce((acc, item) => {
  ...
  const samplesWithTerry = feedItems
    .filter((item) => item.terryCommentary)
    .slice(0, 3);
  if (samplesWithTerry.length > 0) {
    ...
    samplesWithTerry.forEach((item, index) => {
      ...
      console.log(
        `   Source: ${item.source.name} (${Math.round(item.classification.confidence * 100)}% confidence)`,
      );
  ...
  const voiceQualityBreakdown = feedItems
    .filter((item) => item.voiceMetrics)
    .map((item) => item.voiceMetrics);
  if (voiceQualityBreakdown.length > 0) {
    const avgAscerbic =
      voiceQualityBreakdown.reduce((sum, m) => sum + m.ascerbicScore, 0) /
      voiceQualityBreakdown.length;
    const avgBritish =
      voiceQualityBreakdown.reduce((sum, m) => sum + m.britishScore, 0) /
      voiceQualityBreakdown.length;
    const avgFootball =
      voiceQualityBreakdown.reduce((sum, m) => sum + m.footballScore, 0) /
      voiceQualityBreakdown.length;
    const avgGolby =
      voiceQualityBreakdown.reduce((sum, m) => sum + m.golbyScore, 0) /
      voiceQualityBreakdown.length;
