...
const recentITKItems = [
  {
    id: 'itk-1',
    type: 'itk',
    content: 'Arsenal confident about completing Declan Rice deal. Personal terms agreed, medical scheduled.',
...
const partnerContentTemplates = {
  analysis: [
    {
      title: "Tactical Analysis: How {club}'s transfer strategy is reshaping their season",
      content: "Deep dive into {club}'s recent transfer decisions and their tactical implications. The signing of {player} represents a significant shift in approach, with data showing improved possession statistics and defensive stability.",
      tags: ['tactics', 'analysis', 'strategy'],
    },
    {
      title: "Transfer Market Trends: The evolution of player valuations in {year}",
      content: "Comprehensive analysis of how transfer fees have evolved this season, with particular focus on the {club} approach to squad building and long-term planning.",
  ...
  news: [
    {
      ...
      content: "With the window progressing, {club} are still looking to strengthen key areas. Our sources indicate continued interest in {player}, with negotiations expected to intensify.",
      tags: ['transfers', 'updates', 'targets'],
    },
    {
      title: "Behind the Scenes: How modern transfers really work",
      content: "From initial contact to medical completion, we break down the complex process behind modern football transfers, using recent {club} activity as a case study.",
      tags: ['process', 'behind-scenes', 'modern-football'],
    },
  ],
  tactical: [
    {
      title: "Formation Focus: How {club} could line up with their new signings",
      content: "Tactical breakdown of {club}'s potential formations with their summer additions. The arrival of {player} opens up new strategic possibilities and matchup advantages.",
...
function checkQuietPeriod() {
  const now = new Date();
  const lastITKTime = Math.max(...feedItems
...
function shouldMixPartnerContent() {
  ...
  const partnerContentLastHour = feedItems.filter(item => 
    item.type === 'partner' && 
    (Date.now() - new Date(item.timestamp).getTime()) < (60 * 60 * 1000)
  );
  
  if (partnerContentLastHour.length >= 4) {
    return { shouldMix: false, reason: 'Hourly partner content limit reached (4/hour)' };
  }
  
  // Check for recent breaking news (disable mixing during breaking news)
  const recentBreaking = feedItems.some(item => 
    item.type === 'breaking' && 
    (Date.now() - new Date(item.timestamp).getTime()) < (30 * 60 * 1000)
  );
  
  if (recentBreaking) {
    return { shouldMix: false, reason: 'Recent breaking news - not mixing during transfer drama' };
  }
  ...
  const partnerRatio = feedItems.filter(item => item.type === 'partner').length / feedItems.length;
  if (partnerRatio >= 0.25) {
    return { shouldMix: false, reason: 'Partner content ratio limit reached (25% of feed)' };
  }
  
  return { shouldMix: true, reason: 'ITK quiet period detected - ready for partner content' };
}

function generatePartnerContent() {
  // Select random high-credibility partner source
  const highCredibilitySources = partnerSources.filter(s => s.credibility >= 0.85);
  const selectedSource = highCredibilitySources[Math.floor(Math.random() * highCredibilitySources.length)];
  
  // Get trending topics from recent feed
  const trendingClubs = ['Arsenal', 'Real Madrid', 'Manchester United', 'Liverpool'];
  const trendingPlayers = ['Declan Rice', 'Kylian Mbappé', 'Jude Bellingham'];
  
  // Select content template
  const templates = partnerContentTemplates[selectedSource.category] || partnerContentTemplates.news;
  const template = templates[Math.floor(Math.random() * templates.length)];
  
  // Generate content
  const club = trendingClubs[Math.floor(Math.random() * trendingClubs.length)];
  const player = trendingPlayers[Math.floor(Math.random() * trendingPlayers.length)];
...
function generateTerryCommentaryForPartner(partnerContent) {
  ...
  const template = partnerTemplates[Math.floor(Math.random() * partnerTemplates.length)];
...
function simulateContentMixingCycle() {
  ...
  console.log(`   ITK/Breaking items: ${feedItems.filter(item => item.type === 'itk' || item.type === 'breaking').length}`);
  console.log(`   Partner content items: ${feedItems.filter(item => item.type === 'partner').length}`);
  console.log(`   Last ITK activity: ${quietPeriodStatus.minutesSinceLastITK} minutes ago`);
  console.log(`   Quiet period: ${quietPeriodStatus.isQuietPeriod ? '✅ Yes' : '❌ No'}`);
  ...
  console.log(`   Source: ${partnerContent.source.name} (${Math.round(partnerContent.source.credibility * 100)}% credibility)`);
  ...
  if (terryResult) {
    ...
    console.log(`   Voice quality: ${Math.round(terryResult.voiceConsistency * 100)}%`);
  } else {
...
function displayMixingAnalytics() {
  ...
  const last24Hours = partnerItems.filter(item => 
  ...
  console.log(`- Success rate: ${Math.round((mixingStats.successfulMixes / mixingStats.totalChecks) * 100)}%`);
  console.log(`- Last mix time: ${mixingStats.lastMixTime?.toLocaleTimeString() || 'Never'}`);
  
  console.log(`\n📊 Feed Composition:`);
  console.log(`- Total items: ${feedItems.length}`);
  console.log(`- ITK items: ${feedItems.filter(item => item.type === 'itk').length}`);
  console.log(`- Breaking news: ${feedItems.filter(item => item.type === 'breaking').length}`);
  console.log(`- Partner content: ${partnerItems.length}`);
  console.log(`- Partner content ratio: ${Math.round((partnerItems.length / feedItems.length) * 100)}%`);
  ...
  const partnerItemsWithTerry = partnerItems.filter(item => item.terryCommentary);
  if (partnerItemsWithTerry.length > 0) {
    console.log(`\n🎭 Terry on Partner Content:`);
    console.log(`- Partner items with Terry commentary: ${partnerItemsWithTerry.length}`);
    console.log(`- Terry partner commentary rate: ${Math.round((partnerItemsWithTerry.length / partnerItems.length) * 100)}%`);
    ...
    partnerItemsWithTerry.slice(0, 2).forEach((item, index) => {
      ...
      console.log(`      Context: ${item.source.name} - ${item.source.category}`);
    });
  }
  
  console.log(`\n🔄 System Status:`);
  console.log(`- Content mixing: ${mixingStats.successfulMixes > 0 ? '✅ Active' : '⏸️ Inactive'}`);
  console.log(`- Feed balance: ${partnerItems.length / feedItems.length <= 0.25 ? '✅ Within limits' : '⚠️ High partner ratio'}`);
  console.log(`- Quality filter: ${last24Hours.every(item => item.source.credibility >= 0.8) ? '✅ High credibility' : '⚠️ Mixed credibility'}`);
}
...
console.log('🚀 TransferJuice is ready for live global football transfer feed!');