...
async function main() {
  ...
  try {
    ...
    if (!env.TWITTER_BEARER_TOKEN) {
      throw new Error(
        "TWITTER_BEARER_TOKEN is required for real data fetching",
      );
    }
    ...
    console.log(`\n📊 Monitoring Results:`);
    console.log(
      `- Total tweets processed: ${results.reduce((sum, r) => sum + r.tweetsChecked, 0)}`,
    );
    console.log(
      `- Transfer-relevant tweets: ${results.reduce((sum, r) => sum + r.transferRelevant, 0)}`,
    );
    console.log(
      `- High confidence tweets: ${results.reduce((sum, r) => sum + r.highConfidenceTweets, 0)}`,
    );
    ...
    for (const item of highPriorityItems) {
      ...
      await prisma.feedItem.update({
        ...
        data: {
          terryCommentary:
            terryComments[Math.floor(Math.random() * terryComments.length)],
        },
      });
    }

    console.log(
      `🎭 Added Terry commentary to ${highPriorityItems.length} items`,
    );
