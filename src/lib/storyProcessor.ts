...
export function extractPlayerNameSmart(text: string, facts: ExtractedFacts): string | null {
  // Skip non-transfer content
  if (/suffered|injury|injured|fractured|surgery|hospital|medical emergency/i.test(text)) {
    return null;
  }
...
export function extractTransferDetails(text: string, facts: ExtractedFacts): Partial<TransferMetadata> {
  const details: Partial<TransferMetadata> = {};
  
  // Extract FROM and TO clubs
  const leavesMatch = text.match(/leaves\s+([A-Z][a-zA-Z\s]+?)\s+and\s+joins\s+([A-Z][a-zA-Z\s]+?)(?:\s+on)?/);
  ...
  } else if (facts.clubs.length === 1 && /new deal|contract extension|extends|renews/i.test(text)) {
  ...
  } else if (/new deal|contract extension|extends|renews|new contract/i.test(text) && !details.toClub) {
  ...
  const managerMatch = text.match(/(?:manager|coach|boss)\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)/);
  if (managerMatch) {
...
export function calculateStoryImportance(metadata: Partial<TransferMetadata>, facts: ExtractedFacts): number {
  let importance = 0;
  ...
  if (metadata.fee) {
    ...
    if (feeValue >= 100) importance += 5; // £100m+ is massive
    else if (feeValue >= 70) importance += 4; // £70m+ is huge
    else if (feeValue >= 50) importance += 3; // £50m+ is significant
    else if (feeValue >= 30) importance += 2;
    else if (feeValue >= 10) importance += 1;
  }
  
  // Big club involvement
  const megaClubs = ['Real Madrid', 'Barcelona', 'Manchester United', 'Manchester City', 
    'Liverpool', 'Chelsea', 'Arsenal', 'Bayern Munich', 'PSG', 'Juventus'];
  const bigClubs = [...megaClubs, 'Tottenham', 'Atletico Madrid', 'Inter Milan', 
    'AC Milan', 'Borussia Dortmund', 'Newcastle'];
  
  const involvedClubs = [metadata.fromClub, metadata.toClub].filter(Boolean);
  const megaClubCount = involvedClubs.filter(club => megaClubs.includes(club)).length;
  const bigClubCount = involvedClubs.filter(club => bigClubs.includes(club)).length;
...
async function findExistingStory(player: string, clubs: string[]): Promise<any | null> {
...
export async function processRSSIntoStories(items: RSSItem[]) {
  ...
  for (const item of items) {
    try {
      ...
      const feeValue = transferDetails.fee ? parseFloat(transferDetails.fee.replace(/[^0-9.]/g, '')) : 0;
      ...
      const metadata: TransferMetadata = {
        ...
        confidence: facts.isConfirmed ? 'high' : facts.isNearConfirmed ? 'medium' : 'low',
        importance,
      ...
      if (existingStory) {
        ...
        if (existingMeta.status !== metadata.status || 
          ...
          await prisma.story.update({
            ...
            data: {
              metadata: {
                ...existingMeta,
                ...metadata,
                sources: [...new Set([...existingMeta.sources, ...metadata.sources])],
                lastUpdated: new Date(item.date_published)
              }
            }
          });
      ...
      console.log(`Created story: ${playerName} - ${metadata.fromClub || '?'} → ${metadata.toClub || '?'} (${metadata.status})`);
...
function generateTerryComment(metadata: TransferMetadata): string {
  if (metadata.type === 'loan' && metadata.managerMentioned === 'José Mourinho') {
...
export async function getMegaStoriesForRecap(hours: number = 24, minImportance: number = 7) {
