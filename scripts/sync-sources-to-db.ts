...
async function syncSourcesToDatabase() {
  ...
  try {
    ...
    const validUsernames = GLOBAL_ITK_SOURCES.map((s) =>
      s.handle.replace("@", ""),
    );
    console.log(`   Valid sources in code: ${validUsernames.length}`);
    console.log(
      `   - Tier 1: ${GLOBAL_ITK_SOURCES.filter((s) => s.tier === 1 && !s.isShitTier).length}`,
    );
    console.log(
      `   - Comedy: ${GLOBAL_ITK_SOURCES.filter((s) => s.isShitTier).length}`,
    );
    ...
    console.log(
      `   Match: ${finalCount === GLOBAL_ITK_SOURCES.length ? "✅" : "❌"}`,
    );
  } catch (error) {
