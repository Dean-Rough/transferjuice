/**
 * Twitter API v2 Client with Rate Limiting and Terry-Style Error Handling
 * Handles ITK account monitoring with proper error recovery and logging
 */
        ...
        if (attempt < this.retryAttempts) {
          console.warn(
            applyTerryStyle.enhanceMessage(
              `Attempt ${attempt} failed, retrying in ${this.retryDelay}ms: ${error.message}`
            )
          );
