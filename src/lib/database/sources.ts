 ...
 */
export async function getActiveITKSources(options?: {
  tier?: number;
  region?: string;
  ...
  return await prisma.iTKSource.findMany({
    ...
    orderBy: [{ tier: "asc" }, { reliability: "desc" }],
  });
}
...
export async function getSourcesNeedingUpdate(
  ...
  return await prisma.iTKSource.findMany({
    where: {
      isActive: true,
      OR: [{ lastFetchedAt: null }, { lastFetchedAt: { lt: cutoffTime } }],
    },
    orderBy: [
...
export async function updateSourceTiers(): Promise<{
  ...
  for (const source of sources) {
    ...
    const highPriorityRate =
      source.feedItems.filter(
        (item) => item.priority === "HIGH" || item.priority === "BREAKING",
      ).length / source.feedItems.length;
...
export async function deactivateInactiveSources(
  ...
  const result = await prisma.iTKSource.updateMany({
    where: {
      isActive: true,
      OR: [{ lastFetchedAt: { lt: cutoffDate } }, { lastFetchedAt: null }],
    },
    data: {
      isActive: false,
    },
 ...
 */
export async function getSourcesByRegion(region: string): Promise<ITKSource[]> {
  return await prisma.iTKSource.findMany({
    where: {
      region,
      isActive: true,
