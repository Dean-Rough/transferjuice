...
function extractPlayerName(text: string, authorName?: string): string | null {
  ...
  if (/suffered|injury|injured|fractured|surgery|hospital|medical emergency|passed away|died/i.test(text)) {
    return null;
  }
  
  // Remove emojis and special characters first (but keep accented characters)
  const cleanText = text.replace(/[🚨🔴⚪️💣❤️🤍🇸🇪🇧🇷🇫🇷🇩🇪🇮🇹🇪🇸🏴󐁧󐁢󐁥󐁮󐁧󐁿]+/g, '').trim();
  
  // Common club names to filter out
  const clubNames = [
    'Crystal Palace', 'Manchester United', 'Manchester City', 'Real Madrid', 
    'Sporting Lisbon', 'Aston Villa', 'West Ham', 'Newcastle United',
    'Nottingham Forest', 'Leicester City', 'Bayern Munich', 'Atletico Madrid',
    'Paris Saint', 'AC Milan', 'Inter Milan'
  ];
  ...
  const generalMatch = cleanText.match(/\b([A-ZÁÀÂÄÃÅĄČĆĘÈÉÊËĖĮÌÍÎÏŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑÇČŠŽ][a-záàâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšž]+\s+[A-ZÁÀÂÄÃÅĄČĆĘÈÉÊËĖĮÌÍÎÏŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑÇČŠŽ][a-záàâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšž]+)\b/);
  if (generalMatch && generalMatch[1] !== authorName && !clubNames.some(club => generalMatch[1].includes(club))) {
...
export function extractFacts(item: RSSItem): ExtractedFacts {
  ...
  const leavesMatch = text.match(/leaves\s+([A-Z][a-zA-Z\s]+?)\s+and\s+joins\s+([A-Z][a-zA-Z\s]+)/);
  const fromToMatch = text.match(/from\s+([A-Z][a-zA-Z\s]+?)\s+to\s+([A-Z][a-zA-Z\s]+)/);
  ...
  const isWageStory = /pay cut|wage|salary|earning|per week|per year|weekly|annual/i.test(text);
  ...
  if (isWageStory) {
    // Pay cut patterns
    const payCutMatch = text.match(/(?:pay cut|salary cut|wage cut).*?[€£$]([\d.]+)m(?:illion)?/i);
    if (payCutMatch) {
      payCut = payCutMatch[0].match(/[€£$][\d.]+m(?:illion)?/i)?.[0];
    }
    
    // Wage patterns
    const wageMatch = text.match(/[€£$]([\d.]+)(?:k|m)?(?:\s+)?(?:per week|pw|weekly|per year|annually)/i);
  ...
  const isConfirmed = /here we go(?!\s+can\s+be)|confirmed|done deal|completed|official|deal done|medical done|reached TOTAL agreement/i.test(text);
  const isHereWeGo = /here we go(?!\s+can\s+be)/i.test(text); // Special flag for Romano's signature phrase (not "can be soon")
  const isNearConfirmed = /getting closer|can be very soon|TOTAL agreement/i.test(text);
  const isInterest = /interested|monitoring|tracking|considering|eyeing|targeting/i.test(text);
  const isRejected = /rejected|turned down|refused|snubbed/i.test(text);
  
  // Additional player info
  const ageMatch = text.match(/(\d{2})[-\s]?year[-\s]?old/i);
  const goalsMatch = text.match(/(\d+)\s+goals?\s+(?:this\s+)?(?:season|campaign)/i);
  ...
  return {
    ...
    wages,
    payCut,
    contractLength: contractLengthMatch?.[1] ? `${contractLengthMatch[1]} years` : undefined,
...
function determineStoryType(facts: ExtractedFacts[]): AggregatedStory['storyType'] {
  ...
  if (facts.every(f => (f.contractLength || f.contractUntil) && !f.fee)) return 'contract';
  
  // Check for active negotiations - "getting closer", "TOTAL agreement", etc
  if (facts.some(f => f.isNearConfirmed) || facts.some(f => f.fee && !f.isConfirmed)) return 'negotiating';
...
function calculateImportance(story: Partial<AggregatedStory>): number {
  ...
  if (fees.length > 0) {
    ...
    const maxFeeValue = Math.max(...fees.map(fee => {
      const match = fee.match(/([\d.]+)/);
      return match ? parseFloat(match[1]) : 0;
    }));
    
    // Weight based on fee amount
    if (maxFeeValue >= 70) score += 4;      // Mega transfer (like Gyökeres)
    else if (maxFeeValue >= 50) score += 3; // Big transfer
    else if (maxFeeValue >= 30) score += 2; // Significant transfer
  ...
  // Big clubs involved
  const bigClubs = ['Arsenal', 'Chelsea', 'Liverpool', 'Manchester United', 'Manchester City', 'Real Madrid', 'Barcelona', 'Bayern Munich', 'PSG', 'Juventus', 'Atletico Madrid', 'Sporting', 'Sporting CP', 'Porto', 'Benfica'];
  const clubCount = story.primaryClubs?.filter(club => bigClubs.includes(club)).length || 0;
...
export function aggregateStories(items: RSSItem[]): AggregatedStory[] {
  ...
  for (const item of items) {
    const playerName = extractPlayerName(item.content_text, item.authors[0]?.name);
  ...
  return aggregatedStories.sort((a, b) => {
    ...
    return new Date(b.items[0].date_published).getTime() - new Date(a.items[0].date_published).getTime();
  });
}
...
export function compareFacts(facts: ExtractedFacts[]): {
  ...
  } else if (facts.some(f => f.isConfirmed) && facts.some(f => !f.isConfirmed)) {
