...
export async function generateEnrichedNarrative(
  ...
  const yearMatch = tweetText.match(
    /until (\d{4})|(\d{4}) contract|(\d+)yr|(\d+)-year/i,
  );
  const contractLength = yearMatch
    ? yearMatch[1] || yearMatch[2] || yearMatch[3] || yearMatch[4]
    : null;

  // Extract any stats mentioned in the tweet itself
  const goalsMatch = tweetText.match(
    /(\d+)\s+goals?\s+(?:this season|in \d+ games?)/i,
  );
  ...
  if (isContract) {
    ...
    return (
      narrative +
      `The suits upstairs will be patting themselves on the back for this one, while the rest of us wonder if loyalty in football died sometime around 1992.`
    );
  }
  ...
  if (tweetText.includes("striker") || tweetText.includes("forward")) {
    ...
    tweetText.includes("defender") ||
    tweetText.includes("centre-back")
  ) {
  ...
  return (
    narrative +
    `Still, we watch, we wait, and we pretend to be surprised when millionaires move between millionaire clubs for millionaire fees. The beautiful game, ladies and gentlemen.`
  );
}
