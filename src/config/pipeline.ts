...
const EnvironmentConfigSchema = z.object({
  ...
  // Quality Thresholds
  TERRY_VOICE_THRESHOLD: z.string().transform(Number).pipe(z.number().min(0).max(1)).default('0.90'),
  QUALITY_VALIDATION_THRESHOLD: z.string().transform(Number).pipe(z.number().min(0).max(1)).default('0.85'),
  HUMAN_REVIEW_THRESHOLD: z.string().transform(Number).pipe(z.number().min(0).max(1)).default('0.75'),
  
  // Performance Settings
  PROCESSING_TIMEOUT: z.string().transform(Number).pipe(z.number().min(1000)).default('10000'),
  CACHE_TTL: z.string().transform(Number).pipe(z.number().min(60)).default('3600'),
  WEBSOCKET_HEARTBEAT: z.string().transform(Number).pipe(z.number().min(5000)).default('30000'),
  
  // Content Mixing
  QUIET_PERIOD_THRESHOLD: z.string().transform(Number).pipe(z.number().min(1)).default('3'),
  PARTNER_CONTENT_RATIO: z.string().transform(Number).pipe(z.number().min(0).max(1)).default('0.25'),
  ATTRIBUTION_BACKLINK_REQUIRED: z.string().transform(val => val === 'true').default('true'),
...
export const GLOBAL_ITK_SOURCES = {
  tier1: [
    { handle: 'FabrizioRomano', name: 'Fabrizio Romano', region: 'GLOBAL', reliability: 0.95 },
    { handle: 'David_Ornstein', name: 'David Ornstein', region: 'UK', reliability: 0.92 },
    { handle: 'Gianluca_DiMarzio', name: 'Gianluca Di Marzio', region: 'IT', reliability: 0.88 },
  ],
  tier2: [
    { handle: 'Marca', name: 'Marca', region: 'ES', reliability: 0.82 },
    { handle: 'AS_English', name: 'AS English', region: 'ES', reliability: 0.80 },
    { handle: 'lequipe', name: "L'Équipe", region: 'FR', reliability: 0.85 },
    { handle: 'SkySportsNews', name: 'Sky Sports News', region: 'UK', reliability: 0.83 },
  ],
  regional: [
    { handle: 'ESPNBrasil', name: 'ESPN Brasil', region: 'BR', reliability: 0.78 },
    { handle: 'SkySportDE', name: 'Sky Sport DE', region: 'DE', reliability: 0.75 },
    { handle: 'CalcioMercato', name: 'Calciomercato', region: 'IT', reliability: 0.72 },
    { handle: 'FootMercato', name: 'Foot Mercato', region: 'FR', reliability: 0.70 },
  ],
} as const;

// Transfer Keywords by Language
export const TRANSFER_KEYWORDS = {
  english: ['signing', 'deal', 'medical', 'personal terms', 'contract', 'bid', 'fee', 'agreement', 'move', 'join'],
  spanish: ['fichaje', 'traspaso', 'acuerdo', 'cesión', 'contrato', 'oferta', 'clausula'],
  italian: ['trasferimento', 'cessione', 'prestito', 'riscatto', 'contratto', 'offerta'],
  french: ['transfert', 'prêt', 'rachat', 'signature', 'contrat', 'offre'],
  german: ['Wechsel', 'Transfer', 'Leihe', 'Verpflichtung', 'Vertrag', 'Angebot'],
} as const;
...
export const CONTENT_PARTNERS = {
  ...
  fourFourTwo: {
    ...
    categories: ['historical-chaos', 'tactical-deep-dives', 'transfer-retrospectives'],
...
export const PIPELINE_CONFIG = {
  ...
  quality: {
    thresholds: {
      ...
      contentSafety: 0.95,
      legalCompliance: 0.90,
      editorialQuality: 0.80,
      accessibility: 0.85,
    },
...
- Terry-level specificity about absurd details`,
    ...
    quality: {
      factualAccuracyPrompt: 'Check this transfer article for factual accuracy, focusing on transfer fees, player/club names, timeline consistency, and contradictory statements.',
      brandVoicePrompt: 'Evaluate this content for Terry/Joel Golby brand voice consistency, looking for acerbic wit, parenthetical asides, and emotional intelligence.',
      safetyPrompt: 'Check content for safety issues including discriminatory language, potential legal issues, inappropriate content, and harmful misinformation.',
    },
  },
} as const;
...
export const MONITORING_CONFIG = {
  ...
  alerts: {
    ...
    sourceUnavailableThreshold: 0.20, // 20% sources down
    qualityDegradationThreshold: 0.80, // 80% content passing
...
export const SECURITY_CONFIG = {
  ...
  cors: {
    origin: ENV.NODE_ENV === 'production' ? ['https://transferjuice.com'] : true,
    credentials: true,
  },
...
export function validateConfiguration(): { valid: boolean; errors: string[] } {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      errors.push(...error.issues.map(issue => `Environment: ${issue.path.join('.')} - ${issue.message}`));
    }
  }
...
export function printConfiguration() {
  if (ENV.NODE_ENV === 'development') {
    ...
    console.log(`   Terry Voice Threshold: ${CONFIG.quality.thresholds.terryVoice}`);
    console.log(`   Quality Threshold: ${CONFIG.quality.autoPublishThreshold}`);
    console.log(`   Processing Timeout: ${CONFIG.performance.maxProcessingTime}ms`);
    console.log(`   ITK Sources: ${Object.values(CONFIG.sources).flat().length} total`);
    console.log(`   Partner Sources: ${Object.keys(CONFIG.partners).length} partners`);
    console.log(`   Cache TTL: ${CONFIG.performance.cacheTTL}s`);
  }
}
